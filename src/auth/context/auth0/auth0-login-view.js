...
export default function Auth0LoginView() {
  ...
  const WithPopup = useCallback(async () => {
    try {
  ...
  const WithRedirect = useCallback(async () => {
    try {
  ...
  return (
    <>
      ...
      <Stack spacing={2}>
        <Button fullWidth color="primary" size="large" variant="contained" onClick={WithRedirect}>
          Login with Redirect
        </Button>
        ...
        <Button fullWidth color="inherit" size="large" variant="contained" onClick={WithPopup}>
          Login With Popup
        </Button>
